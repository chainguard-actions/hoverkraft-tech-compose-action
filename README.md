# hoverkraft-tech/compose-action

This action runs your compose file(s) and clean up before action finished

Hardened by [Chainguard](https://www.chainguard.dev) from the upstream action at [https://github.com/hoverkraft-tech/compose-action](https://github.com/hoverkraft-tech/compose-action).

## Versions

| Version | Tag | Upstream commit |
|---------|-----|-----------------|
| v3.0.0 | [`v3.0.0`](https://github.com/chainguard-actions/hoverkraft-tech-compose-action/tree/v3.0.0) | [`11beaa1`](https://github.com/hoverkraft-tech/compose-action/commit/11beaa1c2dae4e8ed7b1665aa074723b6cecb0e4) |

## Privacy

This Action contacts Chainguard's licensing server to verify authorization. Connection metadata (IP address, GitHub repository identifier, timestamp, and any metadata encoded in the auth token) is transmitted to Chainguard, Inc. even if authorization is denied in accordance with our [Privacy Notice](https://www.chainguard.dev/legal/privacy-notice)
